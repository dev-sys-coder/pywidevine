...
from construct import BitStruct, Bytes, ConstructError, Container
...
class _Structures:
    magic = b"WVD"  # Removed Const to directly assign the magic

    header = Struct(
        "signature" / Bytes(len(magic)),
        "version" / Int8ub  # Fixed to remove Const and allow setting value directly
    )

    # - Removed vmp and vmp_len as it should already be within the Client ID
    v2 = Struct(
        "signature" / Bytes(len(magic)),
        "version" / Int8ub,  # Fixed as above
        "type_" / CEnum(
            Int8ub,
